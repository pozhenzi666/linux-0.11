...
4. fork子进程，子进程以/etc/rc作为标准输入，然后execve("/bin/sh")让子进程进入到shell并执行/etc/rc
...
一般进程有三态模型（运行、就绪、阻塞）、五态模型、七态模型：

![image-20250818235621676](https://github.com/pozhenzi666/assert/blob/main/images/20250818235621847.png)



linux 0.11状态模型是在5状态模型基础上，合并就绪/运行态、弱化新建/终止态、另外阻塞态拆分为可中断睡眠和不可中断睡眠状态；

![image-20250819000213249](https://github.com/pozhenzi666/assert/blob/main/images/20250819000213378.png)

这么设计的优点如下：

合并就绪/运行态：减少状态切换，时间片用尽时只触发调度就好，不用RUNNING->READY修改状态再切回

阻塞态拆分可中断睡眠和不可中断睡眠：可中断睡眠可以被信号打断，而不可中断睡眠这类关键I/O不被信号打断，能够实现精准控制。

引入僵尸态：退出后保存进程信息供wait回收，避免信息丢失，相比进程退出更安全。

## 进程调度策略

### 调度入口timer_interrupt

调度入口：timer_interrupt --> do_timer

1. 首先在timer_interrupt中，将寄存器压栈；如压入ds/es/fs用来被中断进程的段选择子，压入edx/ecx/eax是因为C语言调用惯例，这几个寄存器是caller-save；压入ebx是因为后面ret_from_sys_call会用到该寄存器，也就是寄存器将被破坏，因此提前压入保护；再加上CPU自动压入的值，最终有如下栈：

![image-20250821000910975](https://github.com/pozhenzi666/assert/blob/main/images/20250821000911101.png)

2. 将ds/es更新为内核数据段选择子，fs设置为用户数据段选择子，然后jiffies值+1
3. 发送EOI给8259A中断控制器，这样将解除它的忙状态，允许新的相同IRQ在上面排队；（此时IF仍被清除，因此不会出现中断抢占）
4. 从0x20处的CS取出当前进程的CPL，将其作为参数传到do_timer，并跳转到do_timer执行
5. 如果cpl=0即当前进程是内核态进程，那么current->stime++，即进程在内核态运行时间+1；否则current->utime++表示当前时间在用户态时间+1
6. 接着判断是否存在定时器，定时器采用差值链表设计；根据定时器到期时间将各定时器串起来，每个定时器中jiffies都是相对上一个定时器到期时间的差值，因此每次timer中断时只需要对链表头的timer的jiffies-1即可；减1后如果jiffies值为0，说明该定时器到期，将触发定时器函数
7. 然后对时间片-1，判断是否还剩时间片；如果还有时间片则返回，不需要调度；如果时间片为0，则看当前进程是否为内核态，是的话也不需要参数调度，否则执行 schedule()进行调度处理

### 调度函数schedule

1. 遍历所有用户任务（排除了idle任务task[0]），检查是否设置了alarm并且alarm到期，是的话将任务的SIGALRM信号位置1
2. 再检查所有任务信号位（排除blocked信号以及sigkill/sigstop信号），如果存在信号并且进程状态是可被中断唤醒（INTERRUPTIBLE），那么将状态置为RUNNING就绪态
3. 循环遍历所有就绪态任务，pick剩余时间片（counter）最大的那个运行
4. 如果所有就绪态任务时间片都为0，那么重新设置所有任务（包括非就绪态任务）的时间片值，计算公式为counter = counter / 2 + priority，然后再到第3步继续pick，如此循环，直到pick出一个任务。**经过这一操作，下次至少可以选出taks[0]任务，因为idle进程状态一直是running**
5. switch_to切到pick出的任务继续运行

### 上下文切换switch_to

switch_to是一段汇编代码，解释如下图所示

![image-20250824210119694](https://github.com/pozhenzi666/assert/blob/main/images/20250824210119832.png)

这个过程就如下图所示

![image-20250824211033771](https://github.com/pozhenzi666/assert/blob/main/images/20250824211033921.png)

### 调度策略

综合上面描述可知，linux 0.11采用的是基于优先级的时间片轮转调度算法，根据优先级动态调整进程的时间片来实现多进程的调度。

这种策略综合了优先级和时间片的因素，对于就绪任务，当前调度被选择执行后时间片减小，下次调度大概率会选择另一个任务，类似于轮转调度，保证了各个任务都有机会执行，避免了某个任务长时间占用CPU。同时，通过对阻塞任务的优先级调整，是的等待时间长的任务在恢复就绪后能够优先得到执行，一定程度上解决了饥饿问题。
